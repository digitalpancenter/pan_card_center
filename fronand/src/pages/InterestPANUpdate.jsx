...
const InterestPANUpdate = () => {
  const [formData, setFormData] = useState({
    ...
    mobile: "",
    email: "",
    mode: "EKYC", // Default to EKYC, uppercase
    category: "INDIVIDUAL",
    panType: "CORRECTION", // Fixed for correction
  });
  ...
  const handleModeChange = (value) => {
    setFormData({ ...formData, mode: value.toUpperCase() });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.panNumber || !formData.name || !formData.dob || !formData.mobile || !formData.email) {
      ...
      <div className="bg-white shadow-lg rounded-xl p-6 w-full max-w-5xl flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-2/3">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input
              type="text"
              value="Change and Correction form (CSF)"
              disabled
            ...
            <select
              ...
              onChange={handleChange}
              className="p-2 border rounded"
              disabled // Disable since it's fixed for correction
            >
              <option value="CORRECTION">Physical PAN Card (Correction)</option>
            </select>
        ...
        <div className="w-full md:w-1/3">
          ...
          <p className="text-red-600 font-semibold mb-4">
            VLE needs to explain the PAN Application process to the consumer before accepting the request. Do not press back or reload during the process.
          </p>
